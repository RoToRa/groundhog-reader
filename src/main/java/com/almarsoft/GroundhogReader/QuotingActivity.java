    ...
    extends Activity
{
	...
	public void onCreate(Bundle savedInstanceState)
	{
		...
		mMultipleFollowup =
		    getIntent().getExtras().getString("multipleFollowup");
		...
		mLinesListView.setAdapter(new QuoteLineAdapter(this,
		    R.layout.banned_item, mQuoteLineItemsList));
	...
	private void done()
	{
		...
		for (int i = 0; i < quoteLinesListLen; i++)
		{
			...
			if (q.imageActive)
			{
				...
				if (i - lastAddedIndex > 1) if (!first)
	...
	// ===================================================================
	private class QuoteLineAdapter
	    extends ArrayAdapter<QuoteLineItem>
	{

		private final ArrayList<QuoteLineItem> items;

		public QuoteLineAdapter(Context context, int textViewResourceId,
		    ArrayList<QuoteLineItem> items)
		{
		    ...
		    ViewGroup parent)
		{
			...
			if (it != null)
			{
				...
				final ImageView banimg =
				    (ImageView)v.findViewById(R.id.img_quoting);

				banimg.setOnClickListener(new OnClickListener() {
					public void onClick(View v)
					{
						imgQuoteLineClicked(position);
					}
				});
