...
public class GroupsCheckAlarmReceiver
    extends BroadcastReceiver
{
	...
	public void onReceive(Context context, Intent intent)
	{
		...
		// Check if we're under wifi and the option to only check under wifi is
		// enabled
		...
		mLock =
		    mgr.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, "GroundhogLock");
		mLock.acquire();
		...
		try
		{
			...
			Method postCallback =
			    this.getClass().getMethod("postCheckMessagesCallBack",
			        postPartypes);

			ServerManager myServerManager = new ServerManager(context);
			mServerMessageGetter =
			    new ServerMessageGetter(this, null, null, postCallback,
			        postCallback, context, myServerManager, 100, false, true);
	...
	public void postCheckMessagesCallBack(String status, Integer resultObj)
	{
		Log
		    .i("Groundhog",
		        "Background checking finished, publishing notification (if needed)");
		...
		{
			Log.w("Groundhog",
			    "Warning: non OK status returned when checking new messages: "
			        + result);
			if (status != null)
		...
		Notification notif =
		    new Notification(R.drawable.icon, total + " new", System
		        .currentTimeMillis());
		...
		if (hasSome)
		{
			...
			PendingIntent contentIntent =
			    PendingIntent.getActivity(mContext, 0, notifyIntent,
