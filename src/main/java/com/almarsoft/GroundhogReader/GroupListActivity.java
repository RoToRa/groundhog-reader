    ...
    extends Activity
{
	...
	// Real name of the groups, used for calling the MessageListActivity with
	// the correct name
	...
	// This is a member so we can interrupt its operation, but be carefull to
	// create it just
	// before the operation and assign to null once it has been used (at the
	// start of the callback, not in the next line!!!)
	...
	public void onCreate(Bundle savedInstanceState)
	{
		...
		Log.d(UsenetConstants.APPNAME, "GroupList onCreate"); //$NON-NLS-1$
		...
		// Group list
		mGroupsList = (ListView)findViewById(R.id.list_groups);
		...
		boolean firstTime = mPrefs.getBoolean("firstTime", true); //$NON-NLS-1$
		...
		mOfflineMode = mPrefs.getBoolean("offlineMode", true); //$NON-NLS-1$

		long expireTime =
		    new Long(mPrefs.getString("expireMode", "86400000")).longValue(); // 1 //$NON-NLS-1$ //$NON-NLS-2$
		// day
		// default
		long lastExpiration = mPrefs.getLong("lastExpiration", 0); //$NON-NLS-1$

		// 0 = manual expiration only. And we check so we don't do more than one
		// expiration a day
		if ((expireTime != 0)
		    && ((System.currentTimeMillis() - lastExpiration) > 86400000))
		    expireReadMessages(false);

		// Reload groups when clicking on "refresh" title button
		ImageView refreshButton = (ImageView)findViewById(R.id.btn_sync_groups);
		...
		// Open the Subscribe activity when clicking on "add" title button
		ImageView addGroupsButton =
		    (ImageView)findViewById(R.id.btn_add_groups);
		addGroupsButton.setOnClickListener(new View.OnClickListener() {
			public void onClick(View v)
			{
				startActivity(new Intent(GroupListActivity.this,
	...
	protected void onResume()
	{
		...
		Log.d(UsenetConstants.APPNAME, "GroupList onResume"); //$NON-NLS-1$
		 ...
		 */
		if (mPrefs.getBoolean("hostChanged", false)) //$NON-NLS-1$
		{
			// The host has changed in the prefs, show the dialog and clean the
			// group headers
			new AlertDialog.Builder(GroupListActivity.this).setTitle(
			    getString(R.string.group_headers)).setMessage(
			    getString(R.string.server_change_detected)).setNeutralButton(
			    "Close", null).show(); //$NON-NLS-1$
		...
		Log.d(UsenetConstants.APPNAME, "onResume, recreating ServerManager"); //$NON-NLS-1$
		...
		// Check if we came here from a notification, offer to sync messages in
		// that case
	...
	protected Dialog onCreateDialog(int id)
	{
		...
		else if (id == ID_DIALOG_UNSUBSCRIBING)
		{
			loadingDialog = new ProgressDialog(this);
			loadingDialog
			    .setMessage(getString(R.string.unsubscribing_deleting_caches));
			loadingDialog.setIndeterminate(true);
			loadingDialog.setCancelable(true);
			return loadingDialog;
		...
		else if (id == ID_DIALOG_MARKREAD)
		{
			loadingDialog = new ProgressDialog(this);
			loadingDialog
			    .setMessage(getString(R.string.marking_read_deleting_caches));
			loadingDialog.setIndeterminate(true);
			loadingDialog.setCancelable(true);
			return loadingDialog;
		}
	...
	public void updateGroupList()
	{
		...
		for (int i = 0; i < count; i++)
		{
			...
			{
				proxyGroupsUnreadCount[i] =
				    builder.append('(').append(unread).append(") ").append(
				        curGroup).toString();
		...
		mGroupsList.setAdapter(new ArrayAdapter<String>(this,
		    R.layout.grouplist_item, mGroupsWithUnreadCountArray));
	...
	public boolean onOptionsItemSelected(MenuItem item)
	{
		...
		case R.id.grouplist_menu_offline:
			...
			else setTitle(getString(R.string.group_online_mode));
			return true;
	...
	{
		new AlertDialog.Builder(GroupListActivity.this).setTitle(
		    getString(R.string.clear_cache)).setMessage(
		    getString(R.string.confirm_expire_messages)).setPositiveButton(
	...
	private void expireReadMessages(boolean expireAll)
	{
		...
		AsyncTask<Boolean, Void, Void> readExpirerTask =
		    new AsyncTask<Boolean, Void, Void>() {
			    ...
			    protected Void doInBackground(Boolean... args)
			    {
				    ...
				    long expireTime =
				        new Long(mPrefs.getString("expireMode", "86400000"))
				            .longValue();
			    ...
			    {
				    // Store the expiration date so we don't do more than once a
				    // day
				    Editor ed = mPrefs.edit();
	...
	public void getAllMessages(boolean getlatest)
	{
		...
		try
		{
			...
			Method successCallback =
			    this.getClass().getMethod("updateGroupList", noargs);

			mServerManager.setFetchLatest(getlatest);
			mDownloader = new GroupMessagesDownloadDialog(mServerManager, this);
			// Even if it is interrupted we update the counters, so we pass the
			// same callback twice
			mDownloader.synchronize(mOfflineMode, groupVector, successCallback,
	    ...
	    ContextMenuInfo menuInfo)
	{
		new MenuInflater(getApplicationContext()).inflate(
		    R.menu.grouplist_item_menu, menu);
	...
	public boolean onContextItemSelected(MenuItem item)
	{
		...
		else if (order == MENU_ITEM_UNSUBSCRIBE)
		{
			...
			new AlertDialog.Builder(GroupListActivity.this).setTitle(
			    getString(R.string.unsubscribe)).setMessage(msg)
			    .setPositiveButton(getString(R.string.yes),
			        new DialogInterface.OnClickListener() {
				        public void onClick(DialogInterface dlg, int sumthin)
				        {
	...
	private void markAllRead(final String group)
	{
		...
		AsyncTask<Void, Void, Void> markAllReadTask =
		    new AsyncTask<Void, Void, Void>() {
			    @Override
			    protected Void doInBackground(Void... arg0)
			    {
				    DBUtils.groupMarkAllRead(group, GroupListActivity.this
	...
	private void unsubscribe(final String group)
	{
		...
		AsyncTask<Void, Void, Void> unsubscribeTask =
		    new AsyncTask<Void, Void, Void>() {
			    @Override
			    protected Void doInBackground(Void... arg0)
			    {
				    DBUtils.unsubscribeGroup(group, GroupListActivity.this
				        .getApplicationContext());
				    return null;
			    }
	...
	// ==================================================================================================
	// OnItem Clicked Listener (start the MessageListActivity and pass the
	// clicked group name
	// ==================================================================================================
	...
	OnItemClickListener mListItemClickListener = new OnItemClickListener() {
		                                               ...
		                                               long id)
		                                           {
			                                           ...
			                                           if (mOfflineMode)
			                                           {
				                                           ...
				                                           // -------------------------------------------------------------------------------------------------
				                                           if (DBUtils
				                                               .groupHasUncatchedMessages(
				                                                   mTmpSelectedGroup,
				                                                   context))
					                                           new AlertDialog.Builder(
					                                               GroupListActivity.this)
					                                               .setTitle(
					                                                   getString(R.string.get_new))
					                                               .setMessage(
					                                                   getString(R.string.warning_online_to_offline_sync))

					                                               .setPositiveButton(
					                                                   getString(R.string.yes_sync),
					                                                   new DialogInterface.OnClickListener() {
						                                                   public void onClick(
						                                                       DialogInterface dlg,
						                                                       int sumthin)
					                                               ...
					                                               .setNegativeButton(
					                                                   getString(R.string.no_enter_anyway),
					                                                   new DialogInterface.OnClickListener() {
						                                                   public void onClick(
						                                                       DialogInterface dlg,
						                                                       int sumthin)
					                                           ...
					                                           new AlertDialog.Builder(
					                                               ...
					                                               .setTitle(
					                                                   getString(R.string.get_new))
					                                               .setMessage(
					                                                   getString(R.string.offline_group_has_no_messages_sync))
					                                               ...
					                                               .show();
				                                           else fetchFinishedStartMessageList();
			                                           ...
			                                           else
			                                           {
				                                           ...
				                                           // ==========================================
				                                           String msg =
				                                               getString(R.string.fetch_headers_question);
				                                           msg =
	...
	private void callDownloaderForMessageList(String group)
	{
		...
		try
		{
			...
			Method successCallback =
			    GroupListActivity.this.getClass().getMethod(
			        "fetchFinishedStartMessageList", noargs);
			Method cancelledCallback =
			    this.getClass().getMethod("updateGroupList", noargs);
			mServerManager.setFetchLatest(false);
			mDownloader =
			    new GroupMessagesDownloadDialog(mServerManager,
			        GroupListActivity.this);
			mDownloader.synchronize(mOfflineMode, groupVector, successCallback,
			    cancelledCallback, GroupListActivity.this);
	...
	public void fetchFinishedStartMessageList()
	{
		mDownloader = null;
		Intent msgList =
		    new Intent(GroupListActivity.this, MessageListActivity.class);
