/*
 * Groundhog Usenet Reader Copyright (C) 2008-2010 Juan Jose Alvarez Martinez This program is free software; you can
	...
	extends Activity
{
	...
	OnItemClickListener mListItemClickListener = new OnItemClickListener()
	{
		...
		public void onItemClick(AdapterView<?> parent, View v, int position, long id)
		{
			...
			if (mOfflineMode)
			{
				...
				{
					new AlertDialog.Builder(GroupListActivity.this).setTitle(getString(R.string.get_new)).setMessage(
						getString(R.string.warning_online_to_offline_sync))

					.setPositiveButton(getString(R.string.yes_sync), new DialogInterface.OnClickListener()
					{
						public void onClick(DialogInterface dlg, int sumthin)
						{
							callDownloaderForMessageList(mTmpSelectedGroup) ;
						}
					}).setNegativeButton(getString(R.string.no_enter_anyway), new DialogInterface.OnClickListener()
					{
						public void onClick(DialogInterface dlg, int sumthin)
						{
			...
			else
			{
				...
				new AlertDialog.Builder(GroupListActivity.this).setTitle(getString(R.string.get_new)).setMessage(msg)
	...
	private void callDownloaderForMessageList(String group)
	{
		...
		try
		{
			...
			Method successCallback =
				GroupListActivity.this.getClass().getMethod("fetchFinishedStartMessageList", noargs) ;
			...
			mDownloader.synchronize(mOfflineMode, groupVector, successCallback, cancelledCallback,
	...
	public boolean onContextItemSelected(MenuItem item)
	{
		...
		if (order == MENU_ITEM_MARKALLREAD)
		{
			...
			new AlertDialog.Builder(GroupListActivity.this).setTitle(getString(R.string.mark_all_read)).setMessage(msg).setPositiveButton(
		...
		else if (order == MENU_ITEM_UNSUBSCRIBE)
		{
			...
			new AlertDialog.Builder(GroupListActivity.this).setTitle(getString(R.string.unsubscribe)).setMessage(msg).setPositiveButton(
				getString(R.string.yes), new DialogInterface.OnClickListener()
				{
					public void onClick(DialogInterface dlg, int sumthin)
					{
	...
	public void onCreate(Bundle savedInstanceState)
	{
		...
		long expireTime = new Long(mPrefs.getString("expireMode", "86400000")).longValue() ; // 1 //$NON-NLS-1$ //$NON-NLS-2$
		// day
		// default
		...
		ImageView addGroupsButton = (ImageView)findViewById(R.id.btn_add_groups) ;
	...
	protected void onResume()
	{
		 ...
		 */
		GroundhogApplication tvApp = (GroundhogApplication)getApplication() ;
		...
		else if (mConfigAlert.isShowing())
		{
			mConfigAlert.hide() ;
		}
		...
		if (mPrefs.getBoolean("hostChanged", false)) //$NON-NLS-1$
		{
			...
			// group headers
			new AlertDialog.Builder(GroupListActivity.this).setTitle(getString(R.string.group_headers)).setMessage(
				getString(R.string.server_change_detected)).setNeutralButton("Close", null).show() ; //$NON-NLS-1$
	...
	{
		new AlertDialog.Builder(GroupListActivity.this).setTitle(getString(R.string.clear_cache)).setMessage(
			getString(R.string.confirm_expire_messages)).setPositiveButton(getString(R.string.yes),
	...
	public void updateGroupList()
	{
		...
		for (int i = 0; i < count; i++)
		{
			...
			{
				proxyGroupsUnreadCount[i] = builder.append('(').append(unread).append(") ").append(curGroup).toString() ;
