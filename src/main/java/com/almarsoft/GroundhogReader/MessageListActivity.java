...
import com.almarsoft.GroundhogReader.lib.Preferences;
    ...
    extends Activity
{
	...
	private ImageView                   mGoGroups, mSync;
	...
	public void onCreate(Bundle savedInstanceState)
	{
		...
		mConfigAlert.setButton(getString(R.string.ok),
		    new DialogInterface.OnClickListener() {
			    ...
			    {
				    startActivity(new Intent(MessageListActivity.this,
		...
		mOfflineMode =
		    mPrefs.getBoolean(Preferences.offlineMode,
		        Preferences.offlineMode_Default);

		// Get the selected group from the GroupListActivity-passed bundle
		mTitleBar = (TextView)findViewById(R.id.topbar_text);
		mGroup = getIntent().getExtras().getString("selectedGroup");
		mGroupID = DBUtils.getGroupIdFromName(mGroup, context);

		mGoGroups = (ImageView)findViewById(R.id.btn_gogroups);
		mGoGroups.setOnClickListener(new OnClickListener() {
			public void onClick(View v)
			{
				MessageListActivity.this.startActivity(new Intent(
				    MessageListActivity.this, GroupListActivity.class));
			}
		});

		mSync = (ImageView)findViewById(R.id.messageList_sync);
		...
		mWakeLock =
		    pm.newWakeLock(PowerManager.SCREEN_DIM_WAKE_LOCK
		        | PowerManager.ON_AFTER_RELEASE, "GroundhogThreading");

		mMsgList = (ListView)findViewById(R.id.list_msgs);
	...
	protected void onStop()
	{
		...
		if ((mLoadDBTask != null)
		    && (mLoadDBTask.getStatus() != AsyncTask.Status.FINISHED))
	...
	protected void onResume()
	{
		...
		// ==================================================================================
		// Detect server hostname or charset changes in the settings (if true,
		// go to the
		// (grouplist activity which will handle better the change, cleanup the
		// headers, etc)
		// ==================================================================================

		if (mPrefs.getBoolean("hostChanged", false)) // The host has changed in
		    // the prefs, go to the
		    // GroupList
		    startActivity(new Intent(MessageListActivity.this,
		...
		// Now get the unread items. Yes, the select to load mHeaderItemsList is
		// done with
		// "WHERE read=0" but the MessageActivity can also set some messages as
		// read using the
	...
	{
		if (mNumUnread == 0)
		    Toast.makeText(this, getString(R.string.no_unread_use_sync),
	...
	/**
	 * Options menu shown with "Menu" key
	 */
	@Override
	public boolean onOptionsItemSelected(MenuItem item)
	{
		...
		case R.id.messagelist_menu_newpost:
			...
			if ((name == null) || (name.trim().length() == 0)
				...
				new AlertDialog.Builder(this)
				    .setTitle(getString(R.string.user_info_unset))
				    .setMessage(
			...
			else
			{
				Intent intent_Compose =
				    new Intent(MessageListActivity.this, ComposeActivity.class);
				intent_Compose.putExtra("isNew", true);
				intent_Compose.putExtra("group", mGroup);
				startActivityForResult(intent_Compose,
		...
		case R.id.messagelist_menu_managebanneds:
			Intent intent_bannedthreads =
			    new Intent(MessageListActivity.this, BannedActivity.class);
			intent_bannedthreads.putExtra("typeban",
			    UsenetConstants.BANNEDTHREADS);
		...
		case R.id.messagelist_menu_managebannedusers:
			Intent intent_bannedusers =
			    new Intent(MessageListActivity.this, BannedActivity.class);
			intent_bannedusers
			    .putExtra("typeban", UsenetConstants.BANNEDTROLLS);
			intent_bannedusers.putExtra("group", mGroup);
			startActivityForResult(intent_bannedusers,
			    UsenetConstants.BANNEDACTIVITYINTENT);
			return true;
	...
	{
		AsyncTask<String, Void, Void> catchupTask =
		    new AsyncTask<String, Void, Void>() {
			    ...
			    protected Void doInBackground(String... groupArr)
			    {
				    ...
				    {
					    Log.w("Groundhog",
					        "Problem catching up with the server");
					    e.printStackTrace();
				    }
			    ...
			    protected void onPostExecute(Void arg0)
			    {
				    Toast.makeText(MessageListActivity.this,
				        R.string.catchup_done, Toast.LENGTH_SHORT);
	...
	void getNewMessagesFromServer(boolean getlatest)
	{
		...
		{
			callback =
			    this.getClass().getMethod("threadMessagesFromDB", noargs);

			mServerManager.setFetchLatest(getlatest);
			mDownloader = new GroupMessagesDownloadDialog(mServerManager, this);
			mDownloader.synchronize(mOfflineMode, groupVector, callback,
			    callback, this);
	...
	{
		new MenuInflater(getApplicationContext()).inflate(
		    R.menu.messagelist_item_menu, menu);
	...
	{
		markThreadAsReadOrUnread(header, true);
		DBUtils.banThread(mGroup, header.getArticle().simplifiedSubject(),
		    getApplicationContext());
		mTitleBar.setText(mGroup + ":" + mNumUnread);
	}

	// setread == true: marks as read, else: marks as unread
	private void markThreadAsReadOrUnread(HeaderItemClass header,
	    boolean setread)
	{
		...
		for (int i = 0; i < headerItemsSize; i++)
		{
			...
			if (thread_subject.equalsIgnoreCase(article.simplifiedSubject()))
			{
				...
				if (setread)
				{
					...
					if (!proxyReadSet.contains(msgId)) proxyNumUnread--;
				}
				else
				{
		...
		mReadSet = DBUtils.getReadMessagesSet(mGroup, getApplicationContext());
		DBUtils.updateUnreadInGroupsTable(mNumUnread, mGroupID,
		    getApplicationContext());
		mMsgList.invalidateViews();
	}
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data)
	{
		...
		if (requestCode == UsenetConstants.COMPOSEMESSAGEINTENT)
		{
			...
			if (resultCode == RESULT_OK) // "new post" (instead of reply)
			    if (mOfflineMode
			        ...
			        getString(R.string.message_sent), Toast.LENGTH_SHORT)
			        .show();
		    ...
		    if (resultCode == RESULT_OK)
			    Toast.makeText(getApplicationContext(),
			        getString(R.string.future_unignored_willbe_fetched),
			        Toast.LENGTH_LONG).show();
		    else if (resultCode == RESULT_CANCELED)
		        Toast.makeText(getApplicationContext(),
		            getString(R.string.nothing_to_unban), Toast.LENGTH_SHORT)
		            .show();
	...
	public boolean onCreateOptionsMenu(Menu menu)
	{
		new MenuInflater(getApplication())
		    .inflate(R.menu.messagelistmenu, menu);
	...
	public boolean onPrepareOptionsMenu(Menu menu)
	{
		...
		else showRead.setTitle(R.string.msglist_group_showread);
	...
	private void markAllRead()
	{
		...
		new AlertDialog.Builder(this).setTitle(
		    getString(R.string.mark_all_read)).setMessage(msg)
		    .setPositiveButton(getString(R.string.yes),
		        new DialogInterface.OnClickListener() {
			        public void onClick(DialogInterface dlg, int sumthin)
			        {
				        if (mHeaderItemsList != null)
				        {
					        ...
					        DBUtils.setGroupAllRead(mGroup,
					            getApplicationContext());
	...
	// Create a thread with big stack so the recursive functions in
	// nntp.Threader.java doesn't crash the stack
	class BigStackThreader
	{

		public BigStackThreader(final Article[] articles)
		    throws InterruptedException
		{
			...
			Thread t =
			    new Thread(new ThreadGroup("tgroup"), r, "threader_thread_",
			        1024 * 1024);
			t.start();
			t.join();
		}
	...
	private class LoadFromDBAndThreadTask
	    extends AsyncTask<Void, Integer, Integer>
	{
		...
		protected void onPreExecute()
		{
			...
			mProgress =
			    ProgressDialog.show(mi, mi.getString(R.string.message), mi
			        .getString(R.string.threading_messages));
		}
		...
		protected Integer doInBackground(Void... arg0)
		{
			...
			String charset =
			    mPrefs.getString("readDefaultCharset", "ISO8859-15");
			DBHelper dbhelper = new DBHelper(getApplicationContext());
			SQLiteDatabase db = dbhelper.getReadableDatabase();

			// Get the msgIds of all my posts to check for replies on
			// fillListNonRecursive; load them on a set

			if (act.mPrefs.getBoolean("markReplies", true))
			    act.mMyPostsSet =
			        DBUtils.getGroupSentMessagesSet(act.mGroup,
			            getApplicationContext());

			// This is not moved to a single function in DBUtils because this
			// way we can update realistically the
			// progressDialog
			...
			for (int i = 0; i < numArticles; i++)
			{
				...
				currentArticle.setFrom(MessageTextProcessor
				    .decodeHeaderInArticleInfo(cur.getString(3), charset));
				currentArticle.setSubject(MessageTextProcessor
				    .decodeHeaderInArticleInfo(cur.getString(4), charset));
			...
			if (articles.length > 0) // This class call the recursive functions
			    // in the message threader inside a thread
			    // with a big stack
		...
		protected void onPostExecute(Integer resultObj)
		{
			...
			case DBGETTER_FINISHED_OK:
				if (mHeaderItemsList != null)
				{
					mMsgList.setAdapter(new ArticleAdapter(
					    MessageListActivity.this, R.layout.messagelist_item,
					    mHeaderItemsList));
					mTitleBar.setText(mGroup + ":" + mNumUnread);
				}
				checkNoUnread();
	...
	private void fillListNonRecursive(Article root, int depth, String replyto)
	{
		...
		ArrayList<HeaderItemClass> nonStarredItems =
		    new ArrayList<HeaderItemClass>();
		HashSet<String> bannedTrollsSet =
		    DBUtils.getBannedTrolls(getApplicationContext());
		HashSet<String> starredSet =
		    DBUtils.getStarredSubjectsSet(getApplicationContext());

		// Proxy for speed
		HashSet<String> myPostsSetProxy = mMyPostsSet;
		ArrayList<HeaderItemClass> headerItemsListProxy =
		    new ArrayList<HeaderItemClass>();
		int refsArrayLen;
			...
			if (!root.isDummy())
			{
				...
				if (!bannedTrollsSet.contains(root.getFrom()))
				{
					...
					if (markReplies)
					{
						...
						if ((msgId != null) && (myPostsSetProxy != null)
						    && myPostsSetProxy.contains(msgId))
							ih.myreply = true;
						else ih.myreply = false;
					}
					...
					else // Nonstarred items will be added to mHeaderItemsList
					// at the end
	...
	private void fillNumbersArray()
	{
		...
		for (int i = 0; i < headerItemsSize; i++)
			numbersArrayProxy[i] =
			    headerItemsProxy.get(i).getArticle().getArticleNumber();
	...
	private class ArticleAdapter
	    extends ArrayAdapter<HeaderItemClass>
	{

		private final ArrayList<HeaderItemClass> items;

		public ArticleAdapter(Context context, int textViewResourceId,
		    ArrayList<HeaderItemClass> items)
		{
		    ...
		    ViewGroup parent)
		{
			...
			if (it != null)
			{
				...
				LinearLayout fullItemLayout =
				    (LinearLayout)v.findViewById(R.id.full_item_layout);

				View indentView = v.findViewById(R.id.indentation_view);
				...
				View subjectLine = v.findViewById(R.id.subjectLine);
				LayoutParams subjectLineLayoutParams =
				    subjectLine.getLayoutParams();

				/*
				 * Check the references of the article. If the first reference
				 * of the article is equal to the last article first reference
				 * (AKA "thread reference") then the subject didn't change
				 */
				...
				final ImageView star =
				    (ImageView)v.findViewById(R.id.img_thread_star);
				...
				// If there are no references in this article or the previous
				// it's a new subject. It is, too,
				// if both have references but the previous article first
				// reference it's different from this article first reference.
				if ((thisArticleReferences.length == 0)
				    || (prevArticleReferences == null)
				    || ((prevArticleReferences != null)
				        && (prevArticleReferences.length > 0) && !prevArticleReferences[0]
				        .trim().equalsIgnoreCase(
				            thisArticleReferences[0].trim())))
				{
				...
				if (subjectChange)
				{
					...
					subjectLine.setLayoutParams(subjectLineLayoutParams);
					...
					if (it.starred)
						star.setImageDrawable(getResources().getDrawable(
						    R.drawable.star_big_on));
					else star.setImageDrawable(getResources().getDrawable(
					    R.drawable.star_big_off));

					TextView subjectLineText =
					    (TextView)v.findViewById(R.id.subjectLine_text);
					subjectLineText.setText(article.getSubject(),
				...
				if (mReadSet.contains(article.getArticleId()))
				...
				else if (!subjectChange) // it.read = false;
	...
	private void itemStarClicked(int position)
	{
		...
		for (int i = 0; i < itemsSize; i++)
		{
			...
			if (starred_thread_subject.equalsIgnoreCase(current.getArticle()
			    .simplifiedSubject())) current.starred = !current.starred;
		}

		DBUtils.updateStarredThread(newValue, starred_thread_subject, mGroupID,
		    getApplicationContext());
		mMsgList.invalidateViews();
	}
}
