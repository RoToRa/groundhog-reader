...
{
	private static final String rfc1738UrlRegExp =
	"(?:http(?:s|)://(?:(?:(?:(?:(?:[a-zA-Z\\d](?:(?:[a-zA-Z\\d]|-)*[a-zA-Z\\d])?)\\.)*(?:[a-zA-Z](?:(?:[a-zA-Z\\d]|-)*[a-zA-Z\\d])?))|(?:(?:\\d+)(?:\\.(?:\\d+)){3}))(?::(?:\\d+))?)"
		+ "(?:/(?:(?:(?:(?:[a-zA-Z\\d$\\-_.+!*'(),]|(?:%[a-fA-F\\d]{2}))|[;:@&=])*)(?:/(?:(?:(?:[a-zA-Z\\d$\\-_.+!*'(),]|(?:%[a-fA-F\\d]{2}))|[;:@&=])*))*)(?:\\?(?:(?:(?:[a-zA-Z\\d$\\-_.+!*'(),]|(?:%[a-fA-F\\d]{2}))|[;:@&=])*))?)?)"
		+ "|(?:ftp://(?:(?:(?:(?:(?:[a-zA-Z\\d$\\-_.+!*'(),]|(?"
	    ...
	    private static final Pattern rfc1738UrlPattern = Pattern.compile(rfc1738UrlRegExp, Pattern.MULTILINE);
	...
	public static String decodeFrom(Field fromField, String charset, Message message)
	{
		...
		if (rawStr.indexOf("=?") != -1)
		{
			...
			if (authorList != null && (author = authorList.get(0)) != null)
				...
				// Parse error on mime4j; some people use emails like "bla@@@bla.com", some stuuuuuupid servers accept
				// them, and mime4j
	...
	/**
	 * Escape a text converting it to HTML. It also convert urls to links; this conversion is not the most advanced: it
	 * only works when the link is entirely contained within the same line and it only converts the first link in the
	 * line. But it covers 95% of cases found on Usenet.
	 * one-line bracketed URLs are now correctly detected. Also now detects other protocols than HTTP.
	 */
	private static String escapeHtmlWithLinks(String line)
	{
		// First escape non-ASCII caracters.
		String result = StringEscapeUtils.escapeHtml(line) ;

		// Escape all non bracketed URLs.
		result = result.replaceAll("((?!<<)[a-z]+://\\S+(?!>>))", "<a href=\"$1\">$1</a>") ;

		// Escape all one-line bracketed URLs.
		result = result.replaceAll("<([a-z]+:.+?)>", "<a href=\"$1\">$1</a>") ;

		return result ;
	}
	
	/*
	 * Changes all RFC 1738 URLs in a block of text (multiline) into HTML links.
	 */
	private static String urlsToLink(String text)
	{
	    return rfc1738UrlPattern.matcher(text).replaceAll("<a href=\"$0\">$0</a>");
	}
	...
	public static String htmlizeFullHeaders(Message message)
	{
		...
		{
			html.append("<strong>From:</Strong> " + "<i>" + escapeHtmlWithLinks(header.getField("From").getBody())
				+ "</i> <br/>\n") ;
		}

		if (header.getField("Subject") != null)
		{
			html.append("<strong>Subject:</Strong> " + "<i>" + escapeHtmlWithLinks(message.getSubject())
				+ "</i><br/>\n") ;
		}

		if (header.getField("Date") != null)
		{
			html.append("<strong>Date:</Strong> " + "<i>" + escapeHtmlWithLinks(header.getField("Date").getBody())
				+ "</i><br/>\n") ;
		}

		if (header.getField("Newsgroups") != null)
		{
			html.append("<strong>Newsgroups:</Strong> " + "<i>"
				+ escapeHtmlWithLinks(header.getField("Newsgroups").getBody()) + "</i><br/>\n") ;
		}

		if (header.getField("Organization") != null)
		{
			html.append("<strong>Organization:</Strong> " + "<i>"
				+ escapeHtmlWithLinks(header.getField("Organization").getBody()) + "</i><br/>\n") ;
		}

		if (header.getField("Message-ID") != null)
		{
			html.append("<strong>Message-ID:</Strong> " + "<i>" + escapeHtmlWithLinks(message.getMessageId())
				+ "</i><br/>\n") ;
		}

		if (header.getField("References") != null)
		{
			html.append("<strong>References:</Strong> " + "<i>"
				+ escapeHtmlWithLinks(header.getField("References").getBody()) + "</i><br/>\n") ;
		}

		if (header.getField("Path") != null)
		{
			html.append("<strong>Path:</Strong> " + "<i>" + escapeHtmlWithLinks(header.getField("Path").getBody())
				+ "</i><br/>\n") ;
		}
		...
		for (int i = 0; i < fieldsLen; i++)
		{
			...
			if (fieldName.equals("From") || fieldName.equals("Subject") || fieldName.equals("Date")
				|| fieldName.equals("Newsgroups") || fieldName.equals("Message-ID") || fieldName.equals("References")
				|| fieldName.equals("Organization") || fieldName.equals("Path"))
			{
				continue ;
			}
			html.append("<strong>" + fieldName + ":" + "</strong>" + " <i>"
				+ escapeHtmlWithLinks(fields.get(i).getBody()) + "</i><br/>\n ") ;
		}
		html.append("<br/><br/>") ;
		return html.toString() ;
	}
	...
	{
	    inputText = urlsToLink(inputText);
	...
	/*
	 * Remove the quoting chars at the start of the lines Works for: ">>>" "> > >" ">> >> > >" etc...
	 */
		...
		for (int i = 0; i < inputTextLen; i++)
		{
			...
			if (i > 1)
			{
				...
				if (charcur == '\n')
				{
					...
					if (charprev != '.' && hasNext && charnext != '>' && charprev != '!' && charprev != ':'
						&& charprev != '?' && !inputText.substring(i - 2, i).equals("> ") && charprev != '>'
						&& charprev != '\n' // Don't remove newlines when there are more than one together (formatting
											// newlines)
						&& hasNext && charnext != '\n')
					{
	...
	private static HashMap<String, String> saveBinaryBody(BinaryBody body, String group)
	{
		...
		String path =
			UsenetConstants.EXTERNALSTORAGE + "/" + UsenetConstants.APPNAME + "/" + UsenetConstants.ATTACHMENTSDIR
				+ "/" + group + "/" ;
		...
		throws IOException
	{
		...
		while ((line = bodyTextReader.readLine()) != null)
		{
			...
			// XXX: ESTO NO SOPORTA UUENCODED SIN PERMISOS!!!
				...
				&& Character.isDigit(sline.charAt(6)) && Character.isDigit(sline.charAt(7))
				&& Character.isDigit(sline.charAt(8)) && Character.isWhitespace(sline.charAt(9))
