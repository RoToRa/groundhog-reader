    ...
    extends PreferenceActivity
{
	...
	public void onCreate(Bundle savedInstanceState)
	{
		...
		mPrefs =
		    PreferenceManager.getDefaultSharedPreferences(OptionsActivity.this);
		addPreferencesFromResource(R.layout.options);

	}

	// ============================================================================================
	// Save the value of host; we'll check it again onPause to see if the server
	// changed to delete
	...
	protected void onResume()
	{
		...
		oldAlarmPeriod =
		    new Long(mPrefs.getString("notifPeriod", "3600000")).longValue();
	...
	protected void checkHostChanged()
	{
		...
		if ((oldHost != null) && (newHost != null))
		    ...
		    {
			    // Host changed, store it in hostchanged so other activities can
			    // detect it
	...
	protected void checkCharsetChanged()
	{
		...
		if ((oldReadCharset != null) && (newReadCharset != null))
		    ...
		    {
			    // Charset changed, store it in charsetChanged so other
			    // activities can detect it
			    Editor editor = mPrefs.edit();
	...
	protected void checkAlarmChanged()
	{
		...
		if ((oldAlarm == false) && (newAlarm == true))
			am.setInexactRepeating(AlarmManager.ELAPSED_REALTIME_WAKEUP,
			    SystemClock.elapsedRealtime() + newAlarmPeriod, newAlarmPeriod,
			    sender);
		else if ((oldAlarm == true) && (newAlarm == false))
			am.cancel(sender);
		else if ((newAlarm == true) && (oldAlarmPeriod != newAlarmPeriod))
