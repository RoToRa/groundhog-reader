...
/**
 * This file is conceptually complex, but can be easily explained. The thing is
 * that the download dialog can be called from different activities (GroupList
 * and MessageList) so the caller must provide a callback so the dialog can
 * return to the caller once it has finished the job. The real network code is
 * not on this class because it would be ugly and also because the background
 * notification service wants to access it too, so to have the classes that
 * really download or post messages (ServerMessageGetter and ServerMessagePost)
 * update us, we in turn provide callbacks to that classes. So this is a little
 * like a callback orgy.
 */
...
public class GroupMessagesDownloadDialog
{
	...
	public void interrupt()
	{
		...
		if (mServerMessageGetter != null) mServerMessageGetter.interrupt();

		if (mServerMessagePoster != null) mServerMessagePoster.interrupt();

		if (mProgressGetMessages != null) mProgressGetMessages.dismiss();
	}

	public void synchronize(boolean offlineMode, final Vector<String> groups,
	    Method callback, Method cancelCallback, Object caller)
	{
	...
	public void preGetMessagesCallBack()
	{
		...
		mProgressGetMessages.setMessage(mContext
	...
	public void progressGetMessagesCallBack(String status, String title,
	    Integer progressCurrent, Integer progressMax)
	{
		if (!mProgressGetMessages.isShowing()) mProgressGetMessages.show();
	...
	public void postGetMessagesCallBack(String status, Integer resultObj)
	{
		...
		case FETCH_FINISHED_OK:
			...
			if ((mCallback != null) && (mCallerInstance != null)) try
			{
				Object[] noparams = new Object[0];
		...
		case FINISHED_ERROR_AUTH:
			new AlertDialog.Builder(mContext).setTitle(
			    mContext.getString(R.string.error)).setMessage(status)
	...
	/**
	 * This calls the ServerMessageGetter, which will create and AsyncTask and
	 * download the messages. We'll provide the callbacks so the
	 * ServerMessageGetter call them on pre/progress and update so we can update
	 * the progress dialog.
	 */
	...
	public void getMessagesFromServer()
	{
		try
		{
			...
			Method preCallback =
			    this.getClass()
			        .getMethod("preGetMessagesCallBack", prePartypes);
			...
			Method progressCallback =
			    this.getClass().getMethod("progressGetMessagesCallBack",
			        progressPartypes);
			...
			Method postCallback =
			    this.getClass().getMethod("postGetMessagesCallBack",
			        postPartypes);

			Method cancelCallback =
			    this.getClass().getMethod("cancelGetMessagesCallBack",
			        (Class[])null);

			mServerMessageGetter =
			    new ServerMessageGetter(this, preCallback, progressCallback,
			        postCallback, cancelCallback, mContext, mServerManager,
			        mLimit, mTmpOfflineMode, false);
	...
	public void prePostMessagesCallBack()
	{
		...
		mProgressPostMessages.setMessage(mContext
		    .getString(R.string.posting_pending_messages));
	...
	public void postPostMessagesCallBack(String status, Integer resultObj)
	{
		...
		if (mProgressPostMessages != null) mProgressPostMessages.dismiss();
	...
	private void postPendingOutgoingMessages()
	{
		...
		{
			Method preCallback =
			    this.getClass().getMethod("prePostMessagesCallBack",
			        new Class[0]);
			...
			postPartypes[1] = Integer.class;
			Method postCallback =
			    this.getClass().getMethod("postPostMessagesCallBack",
			        postPartypes);

			Method cancelCallback =
			    this.getClass().getMethod("postCancelMessagesCallBack",
			        new Class[0]);

			mServerMessagePoster =
			    new ServerMessagePoster(this, preCallback, null, postCallback,
			        cancelCallback, mContext, mServerManager);
