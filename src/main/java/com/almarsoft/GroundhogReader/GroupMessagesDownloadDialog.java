...
/**
 * This file is conceptually complex, but can be easily explained. The thing is that the download dialog can be called
 * from different activities (GroupList and MessageList) so the caller must provide a callback so the dialog can return
 * to the caller once it has finished the job. The real network code is not on this class because it would be ugly and
 * also because the background notification service wants to access it too, so to have the classes that really download
 * or post messages (ServerMessageGetter and ServerMessagePost) update us, we in turn provide callbacks to that classes.
...
public class GroupMessagesDownloadDialog
{
	...
	public GroupMessagesDownloadDialog(ServerManager manager, Context context)
	{
		...
		mWakeLock =
			pm.newWakeLock(PowerManager.SCREEN_DIM_WAKE_LOCK | PowerManager.ON_AFTER_RELEASE, "GroundhogDownloading") ;
	}
	...
	public void getMessagesFromServer()
	{
		try
		{
			...
			Method cancelCallback = this.getClass().getMethod("cancelGetMessagesCallBack", (Class[])null) ;

			mServerMessageGetter =
				new ServerMessageGetter(this, preCallback, progressCallback, postCallback, cancelCallback, mContext,
					mServerManager, mLimit, mTmpOfflineMode, false) ;
	...
	public void postGetMessagesCallBack(String status, Integer resultObj)
	{
		...
		case FINISHED_ERROR_AUTH:
			new AlertDialog.Builder(mContext).setTitle(mContext.getString(R.string.error)).setMessage(status).setNeutralButton(
				close, null).show() ;
			break ;
		}
	...
	private void postPendingOutgoingMessages()
	{
		try
		{
			...
			mServerMessagePoster =
				new ServerMessagePoster(this, preCallback, null, postCallback, cancelCallback, mContext, mServerManager) ;
	...
	public void synchronize(boolean offlineMode, final Vector<String> groups, Method callback, Method cancelCallback,
		Object caller)
	{

		SharedPreferences prefs = PreferenceManager.getDefaultSharedPreferences(mContext) ;
		String maxFetch = prefs.getString("maxFetch", "100") ;
		if (maxFetch.length() == 0)
		{
			maxFetch = "100" ;
		}
